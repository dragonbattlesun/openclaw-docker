# CANSLIM A 股交易纪律系统(v2.2 Runtime 优化版 · 2026-06-21)
...
3. 周线出现底部结构、严格 1B 候选 / 2B 雏形,或平台整理完成。
...
8. 中枢力度:固定 `ZD / ZG / Z`,比较同级别、同方向、围绕同一中枢的离开段 / 回试段 / `Zn` 偏移;力度只作 evidence,不能单独升级买卖点。
9. 自同构递归关系:本级别结论、次级别支持/反证、能否升级、触发位和失败位。
...
| 名词 | 定义 | 实战口径 |
|---|---|---|
| 1B | 本级别下跌趋势背驰后第一买点 | 结构完成后的左侧,只作小仓试错或观察 |
| 2B | 1B 后回踩不创新低 | 右侧确认,early / medium 最佳位 |
...
[原文严格校验 — 人工或脚本扩展]
   ↓
1B 校验:
   ① 本操作级别下跌趋势成立? (a+A+b+B+c,A/B两个同级别中枢,后GG<前DD)
   ② C 段是否相对 A 段创新低且同级别背驰? (MACD 绿柱面积比较)
   ③ 反弹笔是否成立?
   → 三项全过 → 升格为「原文严格 1B」, 仍属结构完成后的左侧,只可按 early/试错执行或等待 2B
...
```
原文严格 1B → A1 standard early 5-8%(结构完成后的左侧,不是确认仓/趋势仓)
...
**命门(原文必备):**
- **1B**:① 本级别 `a+A+b+B+c` 下跌趋势成立,A/B 为两个同级别中枢,后 `GG < 前 DD` ② C 段相对 A 段创新低后**同级别底背驰**(MACD 绿柱面积 c<a) ③ 反弹 Up 笔成立并绑定 C 段低点失效位
...
| reference_chanlun_center_strength.md | §16.2 / §16.4 | 中枢力度 evidence:固定级别、ZD/ZG/Z、同级别比较段、MACD、量能和回试质量,禁止单独升级买卖点 |
...
| reference_canslim_a_share.md | §0-§32 | CANSLIM A 股阈值、M 仓位上限、A/B/C/D、early、突破确认、I/T 口径 |
...
| reference_history_db_repository.md | §36 | history.db / K 线读写统一 repository、标准表/索引、runtime_config 路径、as_of 截断和严格 3B/3S 次级别数据边界 |
| reference_tdx_toolchain.md | §23 / §36 | TDX API 调用顺序、DuckDB、launchd、扫描脚本、回测脚本索引 |
...
共享:`history.db`(访问必须走 `services.history_db`,见 `memory/reference_history_db_repository.md`) / `stock_industry`(数据基础设施)。
...
7. 时间周期只做辅助验证,不能替代形态和能量；量化时代小级别假背驰/假买卖点更多,优先二买/二卖,严格一买/一卖也只按结构完成后的左侧小仓试错。
...
17. 低位启动统一定性为“低位区域 + 右侧触发的执行试错仓”: 低位来自 250 日位置和笔起贴底,触发必须来自起涨笔/30m/60m/1m/5m 确认。它不是原文严格一买,也不能把盘整背驰或中枢下沿回补升级成严格 1B。`tdx/chanlun_low_start.py` 只是 `chanlun_swing_scan` 之上的工程过滤层,stage 只能写 `3B_candidate` / `2B_candidate` / `low_reversal_watch`,不能直接宣称严格递归 1B/2B/3B。底部中枢按 `zg` 的 250 日区间位置判断,DISTRIBUTION 下仓位自动降档,放量只做排序加分。低吸规则必须输出 `practical_plan`,明确能否入场、左右侧属性、仓位档、必退条件和升级条件。
